...
const Signin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if the credentials match the bypass credentials
    if (data.email === "ak@gmail.com" && data.password === "12345678") {
      // Directly sign in without API call for bypass credentials
      Cookies.set("userToken", "bypassToken", { expires: 7, path: "/" });

      toast.success("Login successful with bypass!", {
        className: "font-primary text-sm",
      });
      navigate("/dashboard"); // Redirect to dashboard
      setLoading(false);
